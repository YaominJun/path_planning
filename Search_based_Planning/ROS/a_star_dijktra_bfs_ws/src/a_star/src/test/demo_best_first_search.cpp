...
int main(int argc, char **argv) {
    ros::init(argc, argv, "best_first_search");
    ros::NodeHandle nh("~");
    ...
    while (nh.ok()) {
        ...
        if (start_state_rcv && end_state_rcv) {
            a_star::BestFirstSearch best_first_search(start_state, end_state, grid_map, heuristic_type);
            if (best_first_search.searching(&final_path)) {
                closed_set = *(best_first_search.getClosedSet());
